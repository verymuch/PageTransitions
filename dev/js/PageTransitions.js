(function(){
	var PageTransitions = Widget.extend({

		/* 默认配置 */
		attrs: {
			element: '.pt-perspective', 
			index: -1, 					//初始选中项
			count: 0, 					//选项个数
			loop: false, 				//是否可循环
			direction: 'vertical', 		//默认为垂直
			transitionType: 'slide', 	//切换页面时，过度效果类型，默认为空 为slide
			selectors: {
				// ptWrapper: '.pt-main', 	//各过渡页的父元素的类名
				ptPage: 'pt-page', 		//过渡页的类名
				current: 'pt-page-current', //当前页
			}
		},
		...
		setup: function(){
			...
			//初始化当前配置下的过渡动画类型
			self.initTransitionClass();
			//初始化动画结束事件名、是否正在动画、是否支持css动画
			self.initAminEndEventName();

			//初始化DOM, 页面过渡所有页面
			self.$ptPages = self.$element.children('.' + selectors.ptPage);
			self.set('count', self.$ptPages.length);
			//如果初始页为 -1 且 count > 0 ,将初始页置为0
			if(self.get('count') > 0 && self.get('index') == -1){
				self.set('index', 0);
			}
		...
		initPtPages: function($pages){
			...
			//给当前页添加class 
			$pages.eq(this.get('index')).addClass(this.get('selectors').current);
		},
		...
		*/
		switch: function(inIndex, type){
			...
			if(inIndex > self.get('count')) return false;
			...
			var outIndex = self.get('index'),
				...
				direction = self.get('direction'),
				transitionType = self.get('transitionType');

			//修改动画状态
			self.isAnimating = true;
			/* 修改当前选中项 */
			self.set('index',inIndex);
		...
		/* 根据transitionType和direction的值 生成页面过渡Class */
